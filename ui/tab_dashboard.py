...
def build_dashboard_tab():
    ...
    with ui.column().classes('w-full items-center pt-2'):
        ...
        clock_label = ui.label().bind_text_from(clock_state, 'time_str').classes(
            'text-5xl font-mono font-bold text-blue-600 mt-2 mb-2 shadow-sm rounded px-6 py-2 bg-white border'
        )
        ...
        with ui.row().classes('w-full max-w-5xl justify-center gap-6 items-stretch'):
            ...
            with ui.card().classes('w-96 p-6 shadow-md flex flex-col'):
                ui.label('ℹ️ Статус системи').classes('text-h6 font-bold mb-4')
                ...
                with ui.row().classes('w-full gap-2'):
                    ui.button('Тест', icon='play_circle', color='secondary', on_click=trigger_test_audio).classes(
                        'flex-1')
                    ui.button('Зупинити', icon='stop', color='red', on_click=trigger_stop_audio).classes('flex-1')

            # 3. КАРТКА КЕРУВАННЯ МЕХАНІКОЮ
            with ui.card().classes('w-96 p-6 shadow-md'):
                ui.label('⚙️ Швидка механіка').classes('text-h6 font-bold mb-2')
                ...
                with ui.column().classes('w-full gap-4'):
                    # Рух ВПЕРЕД
                    with ui.row().classes(
                            'w-full justify-between items-center bg-blue-50 p-2 rounded border border-blue-100'):
                        ui.label('Вперед:').classes('font-medium text-sm text-blue-900')
                    ...
                    with ui.row().classes(
                            'w-full justify-between items-center bg-orange-50 p-2 rounded border border-orange-100 mt-2'):
                        ui.label('Назад:').classes('font-medium text-sm text-orange-900')
                        with ui.row().classes('gap-2'):
